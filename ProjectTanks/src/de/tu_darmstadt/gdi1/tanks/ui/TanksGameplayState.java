 ...
 */
public class TanksGameplayState extends BasicGameState {

	private int stateID; // Identifier dieses BasicGameState
	private StateBasedEntityManager entityManager; // zugehoeriger entityManager

	TanksGameplayState(int sid) {
		stateID = sid;
	...
	public void init(GameContainer container, StateBasedGame game)
		...
		background.addComponent(new ImageRenderComponent(new Image(
				"/drop_assets/background.png"))); // Bildkomponente
		...
		esc_pressed.addAction(new ChangeStateAction(TanksLaunch.MAINMENUSTATE));
		...
		mouse_Clicked.addAction(new Action() {
			...
			public void update(GameContainer gc, StateBasedGame sb, int delta,
				...
				drop.setPosition(new Vector2f(gc.getInput().getMouseX(), gc
						.getInput().getMouseY()));

				try {
					// Bild laden und zuweisen
					drop.addComponent(new ImageRenderComponent(new Image(
							"assets/tankPlayer.png")));
				} catch (SlickException e) {
					System.err.println("Cannot find file assets/tankPlayer.png!");
					e.printStackTrace();
				}
				...
				lse.addAction(new ChangeStateAction(TanksLaunch.MAINMENUSTATE));
